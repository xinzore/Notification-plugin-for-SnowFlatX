	...
	class qa_html_theme_layer extends qa_html_theme_base {
		...
		function head_script(){
			...
			if(qa_opt('q2apro_onsitenotifications_enabled') && qa_is_logged_in()) {
				...
				$this->output('<link rel="stylesheet" type="text/css" href="'.QA_HTML_THEME_LAYER_URLTOROOT.'styles.css?1.3.0">');
				...
				if(qa_opt('site_theme')=='SnowFlatX') {
					$this->output('
					<script type="text/javascript">
						$(document).ready(function(){
							// $("#osnbox").detach().appendTo(".qam-main-nav-wrapper");
							var elem = $("#osnbox").detach();
							$(".qam-main-nav-wrapper").prepend(elem);
						});
					</script>
					');
				}
				if(qa_opt('site_theme_mobile')=='SnowFlatX') {
					$this->output('
					<script type="text/javascript">
						$(document).ready(function(){
							// $("#osnbox").detach().appendTo(".qam-main-nav-wrapper");
				...
				if(qa_opt('q2apro_onsitenotifications_rtl')) {
					...
					<style type="text/css">
						...
						/* Snow Flat hacks */
						.qam-main-nav-wrapper #osnbox {
							float: right;
							margin-right:-30px;
						}
						.qam-main-nav-wrapper .nfyWrap {
							top: 31px;
							left: 0;
						}
					</style>
